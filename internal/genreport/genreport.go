...
func NewReportGenerator(contentType string) ReportGenerator {
	...
	base.Pdf.SetHeaderFunc(base.addHeader)
	base.Pdf.SetFooterFunc(base.addFooter)
...
func (b *BaseReportGenerator) addHeader() {
	...
	if b.Branding.LogoURL != "" {
		pdf.ImageOptions(b.Branding.LogoURL, 10, 10, 30, 0, false, fpdf.ImageOptions{ReadDpi: false}, 0, "")
	}
	pdf.SetFont("Arial", "B", 16)
	pdf.SetTextColor(255, 255, 255)
	pdf.SetY(10)
	pdf.SetX(50)
	pdf.CellFormat(150, 10, b.Branding.CompanyName, "", 0, "L", false, 0, "")
	pdf.SetFont("Arial", "I", 12)
	pdf.SetY(20)
	pdf.SetX(50)
	pdf.CellFormat(150, 10, "Report Type: "+b.ContentType, "", 0, "L", false, 0, "")
	pdf.SetY(30)
	pdf.SetX(50)
	pdf.CellFormat(150, 10, "Generated: "+time.Now().Format("2006-01-02 15:04:05"), "", 0, "L", false, 0, "")
	pdf.SetY(45)
	pdf.SetTextColor(int(b.Branding.Theme.Text.R), int(b.Branding.Theme.Text.G), int(b.Branding.Theme.Text.B))
}
...
func (b *BaseReportGenerator) addTitle(title string) {
	pdf := b.Pdf
	pdf.SetFont("Arial", "B", 14)
	accent := b.Branding.Theme.Accent
	pdf.SetFillColor(int(accent.R), int(accent.G), int(accent.B))
	pdf.SetTextColor(255, 255, 255)
	pdf.CellFormat(190, 10, title, "", 1, "L", true, 0, "")
	pdf.SetTextColor(int(b.Branding.Theme.Text.R), int(b.Branding.Theme.Text.G), int(b.Branding.Theme.Text.B))
	pdf.Ln(5)
